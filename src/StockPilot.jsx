...
const catTz=items[0]?.tz;const tzLabel=catTz==="Asia/Seoul"?"KST":catTz==="America/New_York"?"ET":catTz==="Asia/Tokyo"?"JST":catTz==="crypto"?"UTC":catTz==="fx"?"FX":"";
...
<div className="sp-grid4" style={{marginBottom:16}}>{items.map(sym=>{const d=data[sym.key]||{};const up=(d.change||0)>=0;const sparkH=d.history||[];
...
`}</style>
{/* 잠금 오버레이 */}
{!unlocked&&<div style={{position:"fixed",inset:0,zIndex:9999,background:"linear-gradient(135deg,#0B1121,#0F172A 40%,#131C2E)",display:"flex",alignItems:"center",justifyContent:"center"}}>
<style>{`@keyframes shake{0%,100%{transform:translateX(0)}20%,60%{transform:translateX(-8px)}40%,80%{transform:translateX(8px)}}
@keyframes spFadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}`}</style>
<div style={{textAlign:"center",animation:"spFadeIn .5s"}}>
...
<input value={pw} onChange={e=>{const v=e.target.value.replace(/[^0-9]/g,"").slice(0,4);setPw(v);setPwError(false);if(v.length===4){if(v==="0613"){setTimeout(()=>setUnlocked(true),200);}else{setPwError(true);setPwShake(true);setTimeout(()=>{setPwShake(false);setPw("");},500);}}}} type="password" inputMode="numeric" maxLength={4} placeholder="••••" autoFocus style={{width:200,padding:"14px 20px",borderRadius:14,background:"rgba(30,41,59,0.6)",border:pwError?"2px solid #FF6B6B":"2px solid rgba(0,229,160,0.2)",color:"#F8FAFC",fontSize:24,fontWeight:700,textAlign:"center",letterSpacing:12,outline:"none",fontFamily:"monospace",transition:"border .2s"}}/>
</div>
{pwError&&<div style={{marginTop:12,fontSize:12,color:"#FF6B6B",animation:"spFadeIn .3s"}}>비밀번호가 일치하지 않습니다</div>}
