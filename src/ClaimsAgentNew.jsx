...
function TabData(){
  ...
  // ═══ 데이터 구조 맵 정의 ═══
  const DATA_MAP=[
    {cat:"🏗️ 기초 마스터 데이터",color:"#0891b2",desc:"손해사정 업무의 기반이 되는 참조 데이터입니다. 차량 정보, 파손 부위, 부품 가격 등 모든 산정의 기초가 됩니다.",
      items:[
        ...
        {icon:"🔧",name:"파손 부위",key:"DAMAGE_CATS",size:"8카테고리 / 50+부위",detail:"전면부·후면부·좌측면·우측면·상부·하부구조·휠서스펜션·ADAS 등 8개 카테고리로 구분된 50개 이상의 세부 파손 부위 목록입니다.",tab:"견적 산정"},
        ...
        {icon:"📊",name:"파손 정도 배수",key:"sM",size:"4단계",detail:"경미(×0.6) → 중간(×1.0) → 심각(×1.5) → 전손추정(×2.2). 파손 심각도에 따라 견적 금액이 자동 보정됩니다. 수입차는 추가로 ×1.6~3.0이 곱해집니다.",tab:"견적 산정"},
        {icon:"🚦",name:"사고 유형",key:"ACCIDENT_TYPES",size:"10+종",detail:"추돌, 교차로, 차선변경, 주차장, 후진, 유턴, 신호위반, 골목길 충돌 등 10개 이상의 사고 유형 분류입니다.",tab:"전체"},
      ]},
    {cat:"📜 보험 약관 참조",color:"#b45309",desc:"삼성화재(2025.08.16 개정) + 현대해상(2026.08.16 개정) 자동차보험 약관에서 추출한 핵심 보상 기준입니다. AI 분석 시 자동으로 참조됩니다.",
      items:[
        ...
        {icon:"🏥",name:"상해등급별 보험금",key:"injuryGrades",size:"14등급",detail:"자배법 시행령 별표1 기준. 1급(₩2억) ~ 14급(₩1,200만)까지 14개 등급별 책임보험금 한도와 대표 상해 내용. 대인 피해 산정의 핵심 기준입니다.",tab:"대인 피해"},
        {icon:"💐",name:"위자료 기준",key:"consolation",size:"사망/부상 17항목",detail:"사망위자료(65세 미만 ₩8,000만, 65세 이상 ₩5,000만), 장례비(₩500만), 후유장애 위자료 공식, 부상등급별 위자료(1급 ₩1,500만 ~ 11급 ₩250만).",tab:"대인 피해"},
        {icon:"💳",name:"자기부담금",key:"deductible",size:"5항목",detail:"손해액의 20%, 최소 ₩20만 ~ 최대 ₩50만. 전부손해(수리비 ≥ 시가) 시 자기부담금 면제. 고객 상담 시 반드시 안내 필요.",tab:"처리 방법"},
        {icon:"🚗",name:"렌트(대차) 기준",key:"rental",size:"6등급",detail:"경형(₩3~4만/일) ~ 수입대형(₩25~40만/일). 대법원 '동종·동급' 원칙이 핵심. 상위 차종 요구 시 과실심의위 회부 가능.",tab:"처리 방법"},
        {icon:"🔩",name:"품질인증부품",key:"certifiedParts",size:"2025.08.16 개정",detail:"품질인증부품으로 수리 시 OEM부품 공시가격의 25%를 피보험자에게 지급. 경미손상 3유형(도막손상/도장막/범퍼긁힘) 구분 기준 포함.",tab:"견적 산정"},
        {icon:"⚖️",name:"과실비율 기준",key:"faultRules",size:"5개 유형",detail:"교차로(50:50), 차선변경(70:30), 후미추돌(0:100), 좌회전(70:30), 유턴(60~80:20~40). 손해보험협회 과실비율 인정기준 포털 연동.",tab:"과실 산정"},
        {icon:"🍺",name:"음주·무면허 사고부담금",key:"penalties",size:"3유형",detail:"음주운전: 보험금 10~20% 부담 + 자기차량 면책. 무면허: 20% + 면책. 뺑소니: 가중처벌 + 보험사 구상권 행사.",tab:"전체"},
        {icon:"👥",name:"동승자 감액비율",key:"passengerDiscount",size:"5유형",detail:"강요/무단동승 100% 감액, 음주운전 동승 40%, 요청동승 30%, 호의동승 20%, 업무동승 0%(감액 없음).",tab:"대인 피해"},
        {icon:"⚠️",name:"12대 중과실",key:"majorFaults",size:"12개항",detail:"신호위반·중앙선침범·속도위반·횡단보도 등 12개 항목. 종합보험 가입 시에도 형사처벌 면제 불가. 교통사고처리특례법 제3조.",tab:"과실 산정"},
      ]},
    {cat:"🎬 Use Case 시나리오",color:"#7c3aed",desc:"실제 손해사정 업무를 시뮬레이션하는 3가지 프리셋 시나리오입니다. 4개 Agent(견적→대인→과실→처리)가 연계되어 동작합니다.",
      items:[
        {icon:"🏆",name:"Case 1: 교차로 충돌",key:"UC_SCENARIOS.uc1",size:"그랜저 vs BMW 7시리즈",detail:"교차로 골목길 쌍방 충돌. 과실 30:70 vs 70:30 극단 대립, BMW 수리비 ₩25,000,000 과다청구, 대인 5명 동시 처리. 가장 복잡한 종합 시나리오.",tab:"전체 4개 Agent"},
        {icon:"🅿️",name:"Case 2: 주차장 신차 충돌",key:"UC_SCENARIOS.uc2",size:"그랜저 vs GV80(출고3개월)",detail:"100% 자사과실. 타사 GV80 수리비 ₩10,000,000 청구 → AI 적정 ₩6,380,000. 신차 감가 대비가 핵심.",tab:"견적 중심"},
        {icon:"🚧",name:"Case 3: 차선변경 분쟁",key:"UC_SCENARIOS.uc3",size:"차선변경 vs 직진",detail:"타사 10:0 주장 → AI 85:15 판정. 양보험사 협의안 90:10 거부 상황. 과실 분쟁 해결 시나리오.",tab:"과실 중심"},
        {icon:"🔄",name:"A/B차 연동 시스템",key:"CASE_CARS",size:"6세트",detail:"자사 A차 ↔ 타사 B차 탭 전환 시 차량정보·파손부위·파손정도·다이어그램이 모두 연동. 각 Case별 자사/타사 차량 데이터 6세트.",tab:"견적 산정"},
        {icon:"🎯",name:"파손 다이어그램",key:"DAMAGE_CONFIGS",size:"6세트 / 29부위",detail:"SVG 기반 차량 외형 위에 파손 부위를 좌표·색상·비용과 함께 애니메이션으로 표시. 각 부위별 수리비 표시.",tab:"견적 산정"},
        {icon:"🤖",name:"AI 사전응답 프리셋",key:"UC_*",size:"9건 / ~17,700자",detail:"Case 1·2·3에 대한 견적/과실/처리 AI 응답이 사전 정의되어 있어 데모 시 빠르게 시연 가능. 실제 API 미연결 시에도 동작.",tab:"전체"},
      ]},
    {cat:"📊 KPI 관리 데이터",color:"#f59e0b",desc:"손해사정 센터의 실시간 업무 현황을 시뮬레이션합니다. 15초마다 자동 갱신되며, 팀원·Agent·절감실적·우선순위가 모두 연동됩니다.",
      items:[
        {icon:"👥",name:"팀원 구성",key:"TEAM",size:"10명 (1센터장 + 9사정사)",detail:"김영수 센터장 외 9명의 사정사. 각 팀원별 강점(견적/과실/처리/대인)과 약점이 정의되어 배당 최적화에 활용. 민원건 담당자 표시.",tab:"KPI"},
        {icon:"📋",name:"초기 일보 데이터",key:"reports",size:"16건 / 21필드",detail:"RPT-001~016. 견적7건, 과실4건, 처리3건, 대인2건. 민원3건(RPT-002,003,011). 보험사기의심3건. 종결6건, 진행중3건, 협의중4건, 분석완료2건.",tab:"KPI"},
        {icon:"⚡",name:"실시간 배당 생성",key:"CASE_TEMPLATES",size:"4유형 × 112조합",detail:"견적(48조합) + 과실(40) + 처리(12) + 대인(12). 15초마다 liveCount에 맞춰 신규건 추가 + 기존건 상태전환(분석완료→진행중→협의중→종결).",tab:"KPI"},
      ]},
    {cat:"📂 대용량 참조 DB",color:"#6366f1",desc:"4개의 xlsx 파일(총 118,827건)에서 추출한 데이터가 샘플 형태로 임베드되어 있으며, 전체 파일 업로드 시 완전한 데이터로 교체됩니다.",
      items:[
        {icon:"🚗",name:"차량 DB",key:"DATASETS.차량DB",size:"60,329건 / 12컬럼",detail:"1990~2026년 전 차종. 제조사, 모델명, 세대, 연식, 출고가, 차량가액, 연료, 배기량, 차급, 차체유형, 국산/수입 등. 차량 가액 산정의 기초 데이터.",tab:"데이터 뷰어"},
        {icon:"🔧",name:"서비스센터 DB",key:"DATASETS.서비스센터DB",size:"7,846건 / 16컬럼",detail:"전국 자동차 서비스센터. 볼보(35), BMW(25), 벤츠(18) 등 실제 딜러명·주소·전화번호 포함 117건 + 전국 정비소. 제휴센터 안내에 활용.",tab:"데이터 뷰어"},
        {icon:"🚙",name:"렌터카 DB",key:"DATASETS.렌터카DB",size:"2,015건 / 16컬럼",detail:"전국 렌터카 업체 지점 정보. 179개 모델 × 8등급(경형~슈퍼카) 배차 가능 차종. 대차 등급별 일 렌트비 산정 근거.",tab:"데이터 뷰어"},
        {icon:"⚙️",name:"부품 DB",key:"DATASETS.부품DB",size:"48,637건 / 12컬럼",detail:"31개 제조사 × 104차종 × 163세대 × 114부품. 년식/세대별 부품가격, ADAS 레벨, EV/ICE 구분. 엔진·변속기·구동계 등 내부 기계부품 44종 포함.",tab:"데이터 뷰어"},
        {icon:"📜",name:"약관기준 DB",key:"DATASETS.약관기준DB",size:"75건 / 6컬럼",detail:"삼성화재+현대해상 약관에서 추출한 보장종목·상해등급·위자료·자기부담금·렌트기준·품질인증부품·과실비율·사고부담금·동승자감액·12대중과실.",tab:"데이터 뷰어"},
      ]},
  ];
