...
function TabData(){
  ...
  const filtered=ds.sample.filter(row=>search===""||row.some(c=>c&&c.toString().toLowerCase().includes(search.toLowerCase())));
  const totalPages=Math.ceil(filtered.length/PER);
  const paged=filtered.slice(page*PER,(page+1)*PER);
  useEffect(()=>{setPage(0);setSearch("");},[dk]);
  useEffect(()=>{setPage(0);},[search]);
  const cds=[{bg:"#0891b2",lbg:"#ecfeff"},{bg:"#059669",lbg:"#ecfdf5"},{bg:"#7c3aed",lbg:"#f5f3ff"},{bg:"#ea580c",lbg:"#fff7ed"}];
  const cd=cds[dk];
  return(<div style={{height:"100%",display:"flex",flexDirection:"column",gap:10}}>
    {/* 상단 4개 DB 카드 */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
      {DK.map((k,i)=>{const d=DATASETS[k];const c=cds[i];const sel=dk===i;return(
        <div key={k} onClick={()=>setDk(i)} style={{padding:"12px 14px",borderRadius:12,background:sel?c.bg:c.lbg,color:sel?"#fff":c.bg,cursor:"pointer",border:sel?"none":`1.5px solid ${c.bg}30`,transition:"all .2s",boxShadow:sel?`0 4px 12px ${c.bg}40`:"none"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:20}}>{d.icon}</span>
            <span style={{fontSize:10,fontWeight:700,opacity:.7}}>{d.total.toLocaleString()}건</span></div>
          <div style={{fontSize:12,fontWeight:700,marginTop:4}}>{d.title.split("(")[0].trim()}</div>
          <div style={{fontSize:9.5,opacity:.7,marginTop:2}}>{d.desc.length>40?d.desc.slice(0,40)+"...":d.desc}</div>
          {sel&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:6}}>
            {Object.entries(d.stats).map(([k,v])=><span key={k} style={{fontSize:9,background:"rgba(255,255,255,.25)",padding:"2px 6px",borderRadius:4}}>{k}: {typeof v==="number"?v.toLocaleString():v}</span>)}</div>}
        </div>)})}</div>
    {/* 검색 + 페이지 */}
    <div style={{display:"flex",alignItems:"center",gap:10}}>
      <div style={{flex:1,position:"relative"}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={`${ds.title} 검색... (${filtered.length}/${ds.sample.length}건 중 샘플)`}
          style={{width:"100%",padding:"8px 12px 8px 32px",borderRadius:8,border:"1.5px solid #e2e8f0",fontSize:12,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}}/>
        <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",fontSize:13,opacity:.4}}>🔍</span></div>
      <div style={{display:"flex",gap:4,alignItems:"center"}}>
        <button onClick={()=>setPage(Math.max(0,page-1))} disabled={page===0} style={{padding:"5px 10px",borderRadius:6,border:"1px solid #e2e8f0",background:page===0?"#f1f5f9":"#fff",color:page===0?"#94a3b8":"#334155",fontSize:11,cursor:page===0?"default":"pointer",fontFamily:"inherit"}}>◀</button>
        <span style={{fontSize:11,color:"#64748b",minWidth:60,textAlign:"center"}}>{page+1}/{Math.max(1,totalPages)}</span>
        <button onClick={()=>setPage(Math.min(totalPages-1,page+1))} disabled={page>=totalPages-1} style={{padding:"5px 10px",borderRadius:6,border:"1px solid #e2e8f0",background:page>=totalPages-1?"#f1f5f9":"#fff",color:page>=totalPages-1?"#94a3b8":"#334155",fontSize:11,cursor:page>=totalPages-1?"default":"pointer",fontFamily:"inherit"}}>▶</button></div>
      <div style={{fontSize:10,color:"#94a3b8",background:"#f8fafc",padding:"5px 10px",borderRadius:6,border:"1px solid #e2e8f0"}}>
        전체 <b style={{color:cd.bg}}>{ds.total.toLocaleString()}</b>건 중 샘플 {ds.sample.length}행 표시</div></div>
    {/* 테이블 */}
    <div style={{flex:1,overflow:"auto",borderRadius:10,border:"1px solid #e2e8f0",background:"#fff"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,fontFamily:"'DM Mono',monospace"}}>
        <thead><tr style={{position:"sticky",top:0,zIndex:2}}>
          {ds.headers.map((h,i)=><th key={i} style={{padding:"8px 10px",background:cd.bg,color:"#fff",fontWeight:600,fontSize:10,textAlign:"left",whiteSpace:"nowrap",borderRight:"1px solid rgba(255,255,255,.15)"}}>{h}</th>)}
        </tr></thead>
        <tbody>{paged.length===0?<tr><td colSpan={ds.headers.length} style={{padding:40,textAlign:"center",color:"#94a3b8",fontSize:13}}>검색 결과가 없습니다</td></tr>
          :paged.map((row,ri)=><tr key={ri} style={{background:ri%2===0?"#fff":"#f8fafc",transition:"background .1s"}} onMouseEnter={e=>e.currentTarget.style.background="#f0f9ff"} onMouseLeave={e=>e.currentTarget.style.background=ri%2===0?"#fff":"#f8fafc"}>
            {row.map((cell,ci)=><td key={ci} style={{padding:"6px 10px",borderBottom:"1px solid #f1f5f9",whiteSpace:"nowrap",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",color:ci===0?"#94a3b8":"#334155",fontSize:10.5}}>{cell}</td>)}
          </tr>)}</tbody></table></div>
    {/* 하단 범례 */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0"}}>
      <div style={{fontSize:9.5,color:"#94a3b8"}}>💡 본 데이터는 AI 손해사정 시스템의 학습/참조 데이터입니다. 실제 업무 적용 시 최신 데이터를 확인하세요.</div>
      <div style={{display:"flex",gap:8}}>
        {DK.map((k,i)=><span key={k} style={{fontSize:9,color:cds[i].bg,fontWeight:600}}>{DATASETS[k].icon} {DATASETS[k].total.toLocaleString()}</span>)}</div></div>
  </div>);
}
