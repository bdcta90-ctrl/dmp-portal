    ...
    <div style={{display:"flex",flexDirection:"column",height:"calc(100% - "+(useCase&&flow[2]&&stage==="idle"?"108":"54")+"px)"}}>
      ...
      {stage==="result"&&summary&&<div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:12}}>
        ...
        {proposals&&<div style={{animation:"fadeIn .5s"}}>
          {/* ═══ 기존 예상 지출 비교 베이스라인 ═══ */}
          {useCase==="uc1"&&<div style={{...CD,border:"2px solid #fecaca",marginBottom:12,background:"linear-gradient(135deg,#fff5f5,#fefce8)"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontSize:15}}>🚨</span>
                <span style={{fontSize:13,fontWeight:700,color:"#dc2626"}}>기존 예상 지출 (타사 주장 70:30, 협상 전)</span></div>
              <div style={{padding:"4px 12px",borderRadius:8,background:"#dc2626",color:"#fff",fontSize:14,fontWeight:800,fontFamily:"'DM Mono',monospace"}}>₩{(29700000).toLocaleString()}</div></div>
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              {[{l:"B차 수리비 (₩25,000,000 × 70%)",v:17500000,c:"#dc2626"},
                {l:"B차 렌트 — 7시리즈 14일 (₩375,000/일 × 70%)",v:3675000,c:"#f97316"},
                {l:"B차 대인 3명 (₩3,000,000 × 3명 × 70%)",v:6300000,c:"#d97706"},
                {l:"A차 수리비 자사부담 (₩5,000,000 × 30%)",v:1500000,c:"#2563eb"},
                {l:"A차 렌트 (그랜저 동급 14일)",v:725000,c:"#0891b2"},
              ].map((item,i)=><div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 10px",borderRadius:7,background:"rgba(255,255,255,.7)"}}>
                <span style={{fontSize:11,color:"#64748b"}}>{item.l}</span>
                <span style={{fontSize:12,fontWeight:700,color:item.c,fontFamily:"'DM Mono',monospace"}}>₩{item.v.toLocaleString()}</span>
              </div>)}
            </div>
            <div style={{marginTop:8,padding:"6px 10px",borderRadius:7,background:"#fef2f2",border:"1px solid #fecaca",fontSize:11,color:"#991b1b",lineHeight:1.5}}>
              ⚠️ 타사 주장대로 70:30 수용 + 수리비·렌트 원안 그대로 처리 시 <b>₩29,700,000</b> 지출. 아래 3가지 처리안으로 <b>₩12,900,000 ~ ₩16,100,000</b> 절감 가능.</div>
          </div>}
          {/* 처리안 비교 바 */}
          {useCase==="uc1"&&proposals&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}>
            {proposals.map((p,idx)=>{const saving=29700000-parseInt(p.cost.replace(/[₩,]/g,""));const pct=Math.round(saving/29700000*100);
