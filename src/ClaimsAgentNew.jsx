...
| 차량 | 파손 부위 | 수리비 | 비고 |
|------|----------|--------|------|
| **A차 (그랜저)** | 전면 범퍼+본넷+헤드라이트+그릴, 후면 범퍼+트렁크+리어램프 | **₩3,500,000** | 전후면 양쪽 파손 |
...
| **B→A** (B가 A에 배상) | A차 ₩3.5M × B과실 20% | **₩700,000** |
| **B→C** (B가 C에 배상) | C차 ₩4.5M × B과실 20% | **₩900,000** |
| **C→A** (C가 A에 배상) | A차 ₩3.5M × C과실 10% | **₩350,000** |
...
| 보험사 | 지출 (배상) | 회수 | **순부담** |
|--------|-----------|------|----------|
| **A사 (자사)** | ₩7,952,000 + ₩3,150,000 = ₩11,102,000 | ₩700,000 + ₩350,000 = ₩1,050,000 | **₩10,052,000** |
| **B사** | ₩700,000 + ₩900,000 = ₩1,600,000 | ₩7,952,000 + ₩1,420,000 = ₩9,372,000 | **▼ ₩7,772,000** (회수 초과) |
| **C사** | ₩350,000 + ₩1,420,000 = ₩1,770,000 | ₩3,150,000 + ₩900,000 = ₩4,050,000 | **▼ ₩2,280,000** (회수 초과) |
...
> 자사 순부담 **₩10,052,000** + 대인 분담금 예상. 6방향 구상금 정산이 복잡하므로
...
| 차량 | 수리비 | 판정 | 비고 |
|------|--------|------|------|
| A차 (스타리아) | ₩35,000,000 | **전손** | 가액 ₩55M, 수리비/가액 64% → 수리 vs 전손 검토 |
| B차 (트럭) | ₩5,000,000 | 수리 | 후면부 경미 |

> 스타리아 수리비/가액 = 64% → 전손 기준(80%) 미달이나, A필러 변형+전면유리 파손으로 **안전성 사유 전손** 검토 가능
...
| # | 패턴 | 세부 내용 | 위험도 |
|---|------|----------|--------|
| 1 | **6개월 내 3회 접수** | 접수 이력: ₩3.2M → ₩4.8M → ₩4.5M (현재) | 🔴 높음 |
...
| 항목 | 일자 | 비고 |
|------|------|------|
| 사고일 | 2025-01-10 | 태풍 발생일 |
| 청구일 | 2026-02-15 | 접수일 |
| **경과일수** | **400일** | |
| 소멸시효 | **3년** (상법 제662조) | 2028-01-10 만료 |
| **잔여 시효** | **약 690일** | |

> ⚠️ 사고 후 400일 경과 — 시효(3년) 내이나, **조속한 처리 권고**.
...
> 사고 후 400일 경과로 시효(3년) 잔여 690일 — **조속 처리 필요**.
...
function buildContextFromCase(useCase,extraData={}){
  const base={
    ...
    uc6:{accidentType:"3중 추돌 (A→B→C)",vehicleA:"현대 그랜저",vehicleB:"기아 K8",
      ...
      repairCostA:3500000,repairCostB:14200000,repairCostC:4500000,totalCostB:14200000,
      ...
      costFlows:{AtoB:9940000,AtoC:3150000,BtoA:700000,BtoC:900000,CtoA:350000,CtoB:1420000},
      ...
      <div style={{overflowY:"auto"}}>{!rs&&!ld&&<Em text="차량과 파손 부위 선택 후 실행하세요"/>}
        {rs&&<div style={{animation:"fadeIn .4s"}}>
          ...
          <div style={{...CD,border:"1px solid #a5f3fc"}}><div style={{display:"flex",alignItems:"center",gap:7,marginBottom:9}}>
            ...
            <div style={{fontSize:12.5}}>{useCase==="uc1"&&vehTab===1?<RT text={UC1_ESTIMATE_SELF}/>:useCase==="uc1"&&vehTab===2?<RT text={UC1_ESTIMATE_TOTAL}/>:useCase==="uc2"&&vehTab===1?<RT text={UC_ESTIMATE_SELF}/>:useCase==="uc2"&&vehTab===2?<RT text={UC_ESTIMATE_TOTAL}/>:<RT text={tA}/>}</div>
            {/* ═══ 온톨로지 추론 체인 ═══ */}
